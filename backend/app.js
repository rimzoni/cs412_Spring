...
var Model150302007 = require('./model150302007')
...
app.get('/rowByDepartment', (req, res) => {
  ...
  Model150302007.byName(req.query.department)
    .then(function (rows) {
      res.status(200)
      .json(rows)
    })
...
app.get('/rowById', (req, res) => {
  ...
  Model150302007.forge({id: rowId}).fetch().then(function (rows) {
    if (!rows) {
      return res.status(404).json({ error: true, message: 'row not found' })
    } else {
