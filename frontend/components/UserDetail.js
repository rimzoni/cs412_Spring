...
class UserDetail extends Component {
  ...
  /*deleteUserById (userId) {
    axios.post('http://localhost:3000/user/delete?id='+userId)
  ...
  handleSubmit2(e) {
    ...
    axios.get('http://localhost:3000/user/delete?id='+e)
        .then(response => this.props.history.push('/users'))
  }
  ...
  handleChange(event, field) {switch (field) {
      case 'name':
      this.setState(
        {user: {
      ...
      return (
        ...
        <Header />
         ...
         <form onSubmit={this.handleSubmit2}>
          <label>
            Id:
            <input type="integer" name="id" value={this.props.match.params.userId} onChange={(e) => this.handleSubmit2(e, 'id')} />
          </label>
          <br/>
